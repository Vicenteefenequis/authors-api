...
def test_get_full_name(normal_user):
    ...
    excepted_full_name = f"{normal_user.first_name.title()} {normal_user.last_name.title()}"
    assert full_name == excepted_full_name
...
def test_create_superuser_is_not_super_user(user_factory):
    with pytest.raises(ValueError) as err:
